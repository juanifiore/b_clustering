...
iris_cluster <- iris %>%
  ...
  mutate(across(everything(), ~ cut(.x, breaks = 3,
                   labels = c('A', 'B', 'C')))) # discretizamos

# DATASET CARS
car_data <- read.csv("datasets/cars/car.data", header = FALSE)  # Sin nombres de columna
car_data <- na.omit(car_data)
colnames(car_data) <- c("buying", "maint", "doors", "persons", "lug_boot", "safety", "class")

car_cluster <- car_data %>% select(-class)

# DATASET ZOO
zoo_data <- read.csv("datasets/zoo/zoo.data", header = FALSE)  # Sin nombres de columna

colnames(zoo_data) <- c("animal_name", "hair", "feathers", "eggs", "milk", "airborne",
                        "aquatic", "predator", "toothed", "backbone", "breathes",
                        "venomous", "fins", "legs", "tail", "domestic", "catsize", "type")

zoo_cluster <- zoo_data %>% select(-animal_name, -type)  # Excluir nombre y clase

# DATASET GLASS 
glass_data <- read.csv("datasets/glass/glass.data", header = FALSE)  # Sin nombres de columna

colnames(glass_data) <- c("Id", "RI", "Na", "Mg", "Al", "Si", "K", "Ca", "Ba", "Fe", "Type")

glass_cluster <- glass_data %>% select(-Id, -Type) %>%  # Eliminar la columna Species
                 mutate(across(everything(), ~ cut(.x, breaks = 5,
                                    labels = c('A', 'B', 'C', 'D', 'E')))) 

# TESTEAR PARA VARIOS
set.seed(42)
dfs = c(iris_cluster, car_cluster, zoo_cluster, glass_cluster)
distancias = c('KL', 'Wasserstein')
distris = c('unif', 'global')
...
for(df in dfs){
  for(distancia in distancias){
    for(dis in distris){
      res <- clustering(df, 5, distancia, 10, dis)
